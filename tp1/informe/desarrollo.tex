...
Curiosamente, el valor del quantum en el que se estabilizaron result\'o similar para todas las configuraciones, ubic\'andose en torno a los 15 ciclos de duraci\'on. \\ 


El segundo lote utilizado fue uno con tareas que se bloquean una \'unica vez:
...
Esto se repiti\'o para todas las configuraciones de cores.
\begin{center}
	\includegraphics[scale=0.4]{graficos/4cores_loteNoBloqueante.png}
\end{center}

El lote de tareas con mucho bloqueo arroj\'o resultados an\'alogos. \\

Una explicaci\'on posible para los resultados similares entre los distintos lotes es que si bien las configuraciones de las tareas cambi\'o, mantuvimos la configuraci\'on de los tiempos de arribo y duraci\'on de las tareas en el af\'an de obtener resultados comparables. Como las tareas de tipo \textit{taskBatch} se mantienen bloqueadas \'unicamente por un ciclo (por definici\'on del ejercicio 6), las mismas vuelven a la cola de \textit{ready} casi inmediatamente, por lo que el hecho de bloquearse no afecta tanto como podr\'ia suponerse. A su vez, estas tareas se bloquean seguido, por lo que las aquellas que se bloquearon antes recuperan r\'apidamente el uso del CPU.

Lo que observamos en los casos anteriores muestra c\'omo un quantum mayor aumenta el rendimiento de CPU. Esto es as\'i porque proporcionalmente estar\'a m\'as tiempo en ejecuci\'on que realizando cambios de contexto entre tareas. El tiempo de espera disminuye ya que el lote se encontraba diseñado de manera pareja, y las tareas eran de duraciones similares. De esta forma, el mayor quantum favorece a que algunas tareas finalicen antes, eliminando la necesidad de esperar otra vuelta hasta su pr\'oximo turno. \\

Podr\'ian existir casos en los que esto no funcione de igual manera. Si tomamos un lote de tareas donde ingresan primero algunas de muy larga duraci\'on, para luego llegar muchas tareas cortas, encontraremos que al aumentar el quantum estaremos favoreciendo a ls tareas que llegaron primero. Sin embargo, el tiempo de espera de las tareas cortas podr\'ia empeorar. Consideremos el siguiente lote:
